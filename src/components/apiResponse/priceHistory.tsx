...
export default function PriceHistory({
	...
	if (data) {
		...
		data.history.sort((a, b) => a.query_time - b.query_time);
		...
		for (const history of data.history) {
			if (previousHumanDate) {
				...
				while (true) {
					...
					const localEmptyHumanDate = convertToHumanReadable(currentEmpty, { month: "short", day: "numeric" });
			...
			const humanDate = convertToHumanReadable(history.query_time, { month: "short", day: "numeric" });
			previousHumanDate = humanDate;

			graphData.push({
				date: humanDate,
				[LABEL_REGULAR]: centsToHumanString(history.regular_price),
				[LABEL_SALE]: history.sale_price ? centsToHumanString(history.sale_price) : null,
			});

			const localMinPrice = Math.min(history.regular_price, history.sale_price || history.regular_price);
			const localMaxPrice = Math.max(history.regular_price, history.sale_price || history.regular_price)
				...
				<Container>
					<Skeleton visible={isLoading}>
						<Stack ta="center">
							...
							<Text>{"$" + centsToHumanString((data?.min_price.sale_price || data?.min_price.regular_price) || 0)}</Text>
							<Text size="xs" c="dimmed">{convertToHumanReadable(data?.min_price.query_time || 0, { month: "short", day: "numeric", year: "numeric" })}</Text>
						</Stack>
					</Skeleton>
				</Container>
				<Container>
				...
				<Container>
					<Skeleton visible={isLoading}>
						<Stack ta="center">
							...
							<Text>{"$" + centsToHumanString((data?.max_price.sale_price || data?.max_price.regular_price) || 0)}</Text>
							<Text size="xs" c="dimmed">{convertToHumanReadable(data?.max_price.query_time || 0, { month: "short", day: "numeric", year: "numeric" })}</Text>
						</Stack>
					</Skeleton>
				</Container>
			</Flex>
